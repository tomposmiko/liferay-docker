...
function tag_release {
	...
	if (! curl "https://api.github.com/repos/liferay/${repository}/git/refs/tags/${LIFERAY_RELEASE_VERSION}" \
		--header "Accept: application/vnd.github+json" \
		--header "Authorization: Bearer ${LIFERAY_RELEASE_GITHUB_PAT}" \
		--header "X-GitHub-Api-Version: 2022-11-28" \
		| grep "${LIFERAY_RELEASE_VERSION}")
	then
		if (! curl "https://api.github.com/repos/liferay/${repository}/git/tags" \
				--data-raw '
					{
						"message": "",
		...
		then
			...
			lc_log DEBUG "Tag has been added"
		fi
	else
		lc_log ERROR "The tag already exists."
	fi
}
