...
function process_argument_ignore_zip_files {
	...
	IGNORE_ZIP_FILES_CACHE_FILE="${CACHE_DIR}/ignore_zip_files_cache.txt"
...
function process_version_list {
	...
	do
		local zip_list_file="${CACHE_DIR}/list-of-${release_version}.txt"
...
function process_zip_list_file {
	...
	do
		...
		local cache_file="${CACHE_DIR}/${file_url##*://}"
